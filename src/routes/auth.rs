...
async fn login(appdata: Data<AppData>, logincreds: web::Json<UserAuthCredsDTO>) -> impl Responder {
    ...
    jwt.map_or_else(|_x|HttpResponse::Forbidden().body(""), |jwt| {
        jwt.map_or_else(||{HttpResponse::InternalServerError().body("")},|jwt| HttpResponse::Ok().json(json!({"ok":"true","response":jwt})))
    })
}
